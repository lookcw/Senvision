...
for subdir, dirs, files in os.walk(descriptor_dir):
	...
	X=array[:][:,2:-1] #extract the training data without target
	y=array[:][:,-1] #extract target
	#	X, y = X[y != 2], y[y != 2]
	...
	classifier = svm.SVC(kernel='rbf',C=100,probability=True)
	...
	for (train, test), color in zip(cv.split(X, y), colors):
	    ...
	    plt.plot(fpr, tpr, lw=lw, color=color,
		     label='ROC fold %d (area = %0.2f)' % (i, roc_auc))ls
	    i += 1
