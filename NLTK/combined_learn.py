...
for file in files:
	...
	#new array for instances that are at least 4 months recent
	all_featuresets=[]
	...
	for i in range(0,num_folds):
		...
		for n in (featuresets[first_index:second_index]):
			if str(four_months_ago)<str(n[0]):
				print n[0]
		...
		if len(testing_set)!=0:
			...
			normal_acc.append((nltk.classify.accuracy(voted_classifier, testing_set))*(len(testing_set)))
			...
			for i in range(len(dates)):
				...
				#print predictions
			print("voted_classifier accuracy percent:", (nltk.classify.accuracy(voted_classifier, testing_set))*100.0)
	print sum(normal_acc)/float(num_tests)
	if identifier!=0:
		result_writer.writerow([comp_name,sum(normal_acc)/float(num_tests)])
